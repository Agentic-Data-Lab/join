...
加入后默认是**只读成员**：可以读组织内你被授权访问的仓库（目前主要是 `server_info`——服务器文档），
方便你，也方便**让你的 agent 自己去读**这些 Markdown 文档。
如果你参与具体项目，管理员会**按项目**给你开对应仓库的写权限。
...
| --- | --- |
| 中文姓名 | 管理员按名字核对，写全名 |
| **学位** | 硕士生 / 博士生（其他身份请选「其他」并在补充说明里写明，由管理员人工处理） |
| **入学年份** | 四位年份，例如 `2026` |
| 补充说明 | 可选 |
...
- 接受后你会自动进入**年级团队**（`basic-team` 下的 `<两位年><学位>`，例如 2026 级硕士 = `26Master`、
  2025 级博士 = `25PhD`）。年级团队是 `basic-team` 的子团队，**权限完全相同（只读）**，
  只是便于按届通知与统计；团队不存在时会自动创建；
...
- **权限按项目走**：默认只读；参与项目时管理员会给你开对应仓库的写权限。只向你被授权的仓库提交，
  需要改文档请告诉管理员；
...
解析父团队 basic-team → 从 issue 里解析「学位 + 入学年份」→ 算出年级团队名（`26Master` / `25PhD`）→
找不到就用 `parent_team_id=basic-team` **自动创建** → 发出组织邀请（带上年级团队，接受即入组）→
回帖告知（含年级团队名）→ 打 `invited` 标签 → 关闭 issue。
若 issue 里学位/年份缺失或格式不对，工作流会回帖提示并失败，不会发出邀请。
对方点邮件接受后，自动成为只读团队成员。
